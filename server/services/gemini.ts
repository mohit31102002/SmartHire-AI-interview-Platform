...
export async function generateQuestion(
  ...
  if (questionNumber === 4) {
    const behavioralPrompt = `Generate a behavioral interview question for a ${role} position. only one question
    Focus on:
    - Past experiences
...
The question should:
- based on his self introduction extracted the keyword from the introduction and use it for the next question.
- one question of OOPS compulsory.
...
-generate a question that is not too long and not too short.
- generate relevent question for ${role} role.
-do not open code editor for non -technical questions.
...
Return only the question text, without any additional formatting or numbering.`;
  ...
  try {
    ...
    const fallbackQuestions = {
      ...
      "Data Analyst":
        "How would you explain what a database is to a non-technical person?",
    ...
    return (
      fallbackQuestions[role as keyof typeof fallbackQuestions] ||
...
check the answer as the answer is correct or not. if some keyword appears in the answer then it is correct.
accuracy on answers is very important.'
